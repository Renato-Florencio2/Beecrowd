...
A entrada consiste em um número indeterminado de linhas contendo cada uma um valor de 1 até 50. A última linha contém
o valor 0.
Saída
...
int main(){
    ...
    while(cin>>quant && quant != 0){
        ...
        queue<int> fila, desc;
        for(int i = 1 ; i <= quant ; i++) fila.push(i);

        while(fila.size() != 1){
            desc.push(fila.front());
            fila.pop();
